...
myPeer.on("open", (id) => {
    ...
    document.getElementById("join").addEventListener("click", (e) => {
        ...
        currentUserId = id;
        connection.invoke("Join", ROOM_ID, id).catch(function (err) {
            return console.error(err.toString());
        });
...
connection.on("Userconnected", (userId, id) => {
    console.log(userId);
    console.log(id);
    connectToNewUser(id, local_stream);
    user_join(id);
...
document.addEventListener("keydown", (e) => {
    if (e.which === 13 && chatInputBox.value != "") {
        var msg = chatInputBox.value;
        var user = currentUserId;
        connection.invoke("message", ROOM_ID, msg, user);
        chatInputBox.value = "";
    }
});

// nghe tin nhắn được giữ đến và hiển thị lên HTML
connection.on("createMessage", (message, user) => {
    const chat_i = document.getElementById("chat_i");
    let li = document.createElement("li");
    if (user != currentUserId) {
        chat_i.hidden = false;
        li.classList.add("otherUser");
        li.innerHTML = `<div class="mess" id="${currentUserId}" style="border: 0.1 solid;  border-radius: 20px; background: honeydew;"><b  style="font-size: 8px; color: red; margin-left:5px"> user(<small>${user}</small>) </b></br><span style="font-size: 13px; margin-left:10px;"> ${message}</span><div>`;
    } else {
        li.innerHTML = `<div class="mess" id="${user}" style="text-align: right; border: 0.1 solid;  border-radius: 20px; background: moccasin;"><b style="font-size: 8px; color: red; margin-right:5px">you</b></br> <span style="font-size: 13px; margin-right:10px;"> ${message}</span><div>`;
    }
